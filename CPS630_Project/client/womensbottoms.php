...
fetch('http://localhost/CPS630Project/Phase01/server/APIs/getItems.php?' + new URLSearchParams({  gender: '"female"', dept_code: 1}).toString())
.then(res => res.json())
.then(data => {
  data.forEach(item => {
  ...
  function removeItem(e) {
	...
	removeEventListeners(items); // Remove event listeners before adding them again
        ...
        .catch(error => {
		 ...
		 <h1 style="background-color: #f0f7eeff; padding: 5px;">Womens Bottoms</h1>
